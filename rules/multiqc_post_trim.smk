rule multiqc_post_trim:
    ...
    output:
        report("qc/post_trim_multiqc_report.html", caption = "../report/qualitychecks.rst", category = "Quality checks", subcategory = "Post-trimming")
    conda:
        "../envs/multiqc.yaml"
    message:
        "Searching for analysis logs to compile a HTML report"
    shell:
        "multiqc {input.zips} -o {output} -n post_trim_multiqc"